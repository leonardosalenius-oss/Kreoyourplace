# KREO V36.07 PRESENZE UNIFICATE - generated by ChatGPT
...
def upload_documento_cliente(cliente_id, file, tipo_documento, note_documento=""):
    """
    V36.07: ripristina la logica documenti che funzionava.
...
def v36_registra_presenza(cliente_id, accesso_id=None, tipo_presenza="PRESENZA_VALIDA", scala_lezione=True, motivo="", note=""):
    """
    V36.07: il check-in usa il motore unico presenze.
    Prima il bottone autorizzava ma il residuo poteva non cambiare perché salvava solo presenze_v36/ledger non coerente.
    Ora:
    - se scala=True passa da kreo_gestisci_presenza()
    - registra SCALA coerente
    - forza ricalcolo ledger cliente
    - aggiorna accesso_tornello
    """
    try:
        sb = get_supabase()
        accesso_row = {}
        if accesso_id not in [None, "", "nan"]:
            try:
                rows = sb.table("accessi_tornello").select("*").eq("id", int(float(accesso_id))).limit(1).execute().data or []
                accesso_row = rows[0] if rows else {}
            except Exception:
                accesso_row = {}

        motivo_finale = motivo or tipo_presenza or "Presenza autorizzata da Check-in V36"

        # Motore unico, se presente.
        if "kreo_gestisci_presenza" in globals():
            ok, msg = kreo_gestisci_presenza(
                cliente_id=cliente_id,
                accesso_id=accesso_id,
                accesso_row=accesso_row,
                azione="CONFERMA",
                consuma_lezione=bool(scala_lezione),
                motivo=motivo_finale,
                source="CHECKIN_V36",
            )
            # Forza ricalcolo da ledger, così Cliente 360 e snapshot si aggiornano subito.
            if scala_lezione and "kreo_recalcola_contatori_ledger_cliente" in globals():
                kreo_recalcola_contatori_ledger_cliente(cliente_id)
            return ok, msg

        # Fallback se per qualche motivo il motore unico non esiste.
        tipo_mov = "SCALA" if scala_lezione else "PRESENZA_NO_SCALA"
        if scala_lezione:
            v36_registra_ledger(cliente_id, -1, tipo_mov, motivo=motivo_finale, accesso_id=accesso_id, note=note)
        else:
            v36_registra_ledger(cliente_id, 0, tipo_mov, motivo=motivo_finale, accesso_id=accesso_id, note=note)

        if accesso_id not in [None, "", "nan"]:
            try:
                sb.table("accessi_tornello").update({
                    "stato_accesso": "PRESENZA_CONFERMATA_SCALA" if scala_lezione else "PRESENZA_AUTORIZZATA_NO_SCALA",
                    "note": f"{motivo_finale} | {note or ''}".strip(" |"),
                }).eq("id", int(float(accesso_id))).execute()
            except Exception:
                pass
        ...
        return True, "Presenza registrata e contatori aggiornati."
    except Exception as e:
        return False, f"Errore registrazione presenza V36.07: {e}"
...
# ============================================================
# KREO V36.07 - Reception navigation safe router
# ============================================================
...
def kreo_reception_open(view_name):
    """
    V36.07: apre sottoviste senza toccare chiavi widget del menu laterale.
    """
...
def kreo_render_back_to_reception(label="⬅️ Torna alla Reception"):
    """
    V36.07: disattivato nelle sottoviste per evitare doppio bottone.
...
def main():
    # V36.07 safe return bridge: handled before menu rendering when possible.
    ...
    with col_title:
        ...
        st.caption(f"Database cloud Supabase | Accesso: {user_label()} | Ruolo: {current_user().get('ruolo', '') if current_user() else ''} | Launch Stable V36.07")
...
# ============================================================
# KREO V36.07 - Final safe Reception router override
# ============================================================

def kreo_reception_clear_navigation():
    """
    Override finale V36.07.
