# KREO V37.03 PRESENZE UNIFICATE - generated by ChatGPT
...
def upload_documento_cliente(cliente_id, file, tipo_documento, note_documento=""):
    """
    V37.03: ripristina la logica documenti che funzionava.
...
def v36_registra_presenza(cliente_id, accesso_id=None, tipo_presenza="PRESENZA_VALIDA", scala_lezione=True, motivo="", note=""):
    """
    V37.03: registra la presenza da accesso con anti-duplicato.
    ...
    try:
        ...
        return False, f"Errore registrazione presenza V37.03: {e}"
...
# ============================================================
# KREO V37.03 - Presenze anti-duplicato / accesso chiuso
# ============================================================
...
# ============================================================
# KREO V37.03 - Reception navigation safe router
# ============================================================
...
def kreo_reception_open(view_name):
    """
    V37.03: apre sottoviste senza toccare chiavi widget del menu laterale.
    """
...
def kreo_render_back_to_reception(label="⬅️ Torna alla Reception"):
    """
    V37.03: disattivato nelle sottoviste per evitare doppio bottone.
...
def main():
    # V37.03 safe return bridge: handled before menu rendering when possible.
    ...
    with col_title:
        ...
        st.caption(f"Database cloud Supabase | Accesso: {user_label()} | Ruolo: {current_user().get('ruolo', '') if current_user() else ''} | Launch Stable V37.03")
...
# ============================================================
# KREO V37.03 - Final safe Reception router override
# ============================================================

def kreo_reception_clear_navigation():
    """
    Override finale V37.03.
...
# ============================================================
# KREO V37.03 - Operatività reale: no cumulativo, agenda giorni,
...
def is_pacchetto_settimanale_kreo(pacchetto):
    """V37.03: logica cumulativa disattivata. I pacchetti NON maturano più lezioni automatiche."""
    return False


def kreo_is_pacchetto_standard_cumulativo(pacchetto):
    """V37.03: cumulativo disattivato."""
    return False
...
def lezioni_maturate_cumulative(cliente, data_ref=None):
    """V37.03: nessuna maturazione settimanale. Torna alle lezioni contrattuali/manuali."""
    try:
...
def contatori_cumulativi_cliente(cliente_id, data_ref=None, pacchetto=None):
    """V37.03: contatori semplici da tabella clienti."""
    try:
        c = get_cliente(int(float(cliente_id)))
        totale = int(float(c.get("numero_lezioni") or c.get("numero_lezioni_totali") or 0))
        usate = int(float(c.get("lezioni_utilizzate") or 0))
...
def aggiorna_contatori_cumulativi_clienti():
    """V37.03: disattivato per evitare modifiche automatiche ai residui."""
    return {"aggiornati": 0, "saltati": 0, "personalizzati_ignorati": 0, "disattivato": True}


def render_lezioni_cumulative_admin():
    st.info("Ricalcolo lezioni cumulative disattivato in V37.03. Le lezioni si gestiscono da pacchetto, presenze, rettifiche e incassi.")


def render_recalcolo_settimanale_widget():
    st.info("Ricalcolo settimanale disattivato in V37.03.")


def dataframe_clienti_con_contatori_cumulativi(df):
    """V37.03: non altera i contatori. Ritorna la tabella così com'è."""
    return df


def kreo_document_bucket_status():
    """
    V37.03: ripristino logica che funzionava.
...
def signed_document_url(storage_path, expires_in=300):
    """V37.03: per i documenti usiamo URL pubblico su bucket documenti; signed URL solo fallback."""
    if not storage_path:
        return ""
    try:
...
def load_documenti(cliente_id=None):
    """V37.03: tabella storica funzionante: documenti_cliente."""
    sb = get_supabase()
    data = []
    try:
        q = sb.table("documenti_cliente").select("*").order("id", desc=True)
...
def upload_documento_cliente(cliente_id, file, tipo_documento, note_documento=""):
    """V37.03: upload coerente col vecchio storico: bucket documenti + tabella documenti_cliente + public_url."""
    if file is None:
        return False, "Nessun file caricato."
...
def kreo_recalcola_contatori_ledger_cliente(cliente_id):
    """V37.03: niente cumulativo e niente ricalcolo da ledger storico. Usa snapshot cliente."""
    try:
        cliente = get_cliente(int(float(cliente_id)))
        totale = int(float(cliente.get("numero_lezioni") or 0))
        usate = int(float(cliente.get("lezioni_utilizzate") or 0))
...
def kreo_decidi_accesso_una_volta(cliente_id, accesso_id, scala_lezione=True, motivo=""):
    """
    V37.03: autorizzazione presenza semplice.
    ...
    try:
        ...
        tipo = "PRESENZA_SCALATA" if scala_lezione else "PRESENZA_NO_SCALA"
        try:
            v36_registra_ledger(cliente_id, delta, tipo, motivo=motivo, accesso_id=accesso_id, note="V37.03 scalaggio semplice")
        except Exception:
            pass
        try:
            get_supabase().table("clienti").update({
        ...
        return False, f"Errore decisione accesso V37.03: {e}"
...
# ============================================================
# KREO V37.03 - Fix lezioni residue + Modifica accessi UX
# ============================================================
...
def kreo_recalcola_contatori_ledger_cliente(cliente_id):
    """
    Override V37.03: niente cumulativo, niente colonna lezioni_residue.
...
def kreo_correggi_decisione_accesso(cliente_id, accesso_id, nuova_azione, motivo="Correzione decisione accesso"):
    """
    Override V37.03: correzione intuitiva e coerente con colonne reali.
    """
    try:
        cliente = v3702_get_cliente_safe(cliente_id)
        if not cliente:
...
def render_v37_modifica_accessi():
    """
    V37.03: vista non tecnica, senza JSON visibile.
    """
    st.header("🔑 Modifica accessi")
...
# ============================================================
# KREO V37.03 - Fix cliente senza lezioni_residue + Agenda Luxury operativa
# ============================================================
...
def render_v37_nuovo_cliente():
    """
    Override V37.03: nuovo cliente senza colonna lezioni_residue.
    """
    st.header("➕ Nuovo cliente")
...
def render_staff_modifica_cliente():
    """
    Override V37.03: modifica cliente senza lezioni_residue.
...
def render_v37_agenda_7gg():
    """
    Override V37.03: Agenda Luxury operativa.
